...
- [x] 1.6 Task decomposition strategies — Decompose along the axis matching task structure (stage=sequential pipeline / data=parallel fan-out / role=functional), only when a real trigger fires (context overflow, independent parallel work, distinct expertise/perspective, checkpointing) — at the coarsest granularity giving independent, verifiable units. First decision is "split at all?" (default: simplest loop). Map dependencies first: strict chains must be sequenced (parallelizing them = false parallelism), independent items fan out. Real tasks = a DAG (directed, acyclic — no cycles; review loops bounded in harness, not as graph cycles); execution waves fall out of it. Splitting is a quality/latency/context tool, never cost-savings (~15×). Synthesis (fan-in) is first-class hard work (shared output contract + real reconciliation). Match enforcement to stakes (structural gating for irreversible steps). Independent review = fresh-context reviewer as downstream node, never a same-agent self-check.
- [x] 1.7 Session state, resumption, forking — API is stateless; a "session" is the harness-persisted `messages` array (+ system prompt/tools/config); external side effects live in the real world, NOT the array. Resume = continue the SAME timeline later (one head; Claude Code `--continue`/`--resume` reload the on-disk JSONL transcript, per-directory) — replays text, does NOT re-run tools, and the world may have drifted (re-observe on resume). Fork = COPY the array into an independent branch (two heads sharing a prefix; `--fork-session` writes a new session ID, preserving the parent as a clean fallback). Forking copies the conversation, not the world — branches' commits/emails really happen, so back irreversible actions with real isolation (worktrees/sandboxes). Fork (continue as same agent, full history) ≠ subagent (isolated fresh context, brief-only, returns a string). Sessions form a DAG; branches don't auto-merge — a synthesizer reconciles. Caching is content-addressed + TTL-bound (~5min/1hr): forks sharing a prefix hit ONE cheap cached prefix (≈0.1×) if run promptly; resuming cold (idle past TTL) pays a one-turn cache re-write (~1.25×) then re-amortizes. Forking cuts price-per-token of the shared prefix, not the token count.
